...
export function computeMatchScore(a: User, b: User): number {
  ...
  const interestsOverlap = a.interests.filter((i) => b.interests.includes(i)).length;
  ...
  if (a.prefSameSemesterOrYear && (a.semester === b.semester || a.universityYear === b.universityYear)) {
    score += 5;
  }
  if (b.prefSameSemesterOrYear && (a.semester === b.semester || a.universityYear === b.universityYear)) {
    score += 5;
  }
...
export async function ensureMatchBetween(userId: string, otherId: string) {
  ...
  const reverse = await prisma.match.findUnique({
    where: { initiatorId_receiverId: { initiatorId: b.id, receiverId: a.id } }
  });
